...
First of all, I would like to extend my sincere gratitude to my supervisor Edwin Blum. He trusted me and gave me full support even when I had not much knowledge about the robotic system. And I would like to express my thanks to all my colleagues who guided me through the whole process. Also many thanks to wbk who offered me access to the PTL, the UR10e Cobot, and the infrastructure at the institute.

On a personal note, I want to thank my family who offered me the comfort and conditions to study in Germany over the past 4 years. Many thanks to the friends I got to know here, whom I know would always stand by me and help me out.

In conclusion, I am grateful to everyone, who contributed to this significant milestone in my life. This journey in Germany has lasted for 4 years. I believe this thesis is a very good closing of what I have learned and achieved during this long journey. 
...
\begin{itemize}
  \item \textbf{Claude Code (Anthropic):} Used for academic writing assistance, including prose editing and refinement, structural feedback on chapter organisation, simulated peer-review commentary for revision guidance, and numerical consistency checking across chapters. All AI-assisted text was reviewed against experimental data and either accepted, revised, or rejected by the author before inclusion.
\end{itemize}

The following components were performed exclusively by the author without AI assistance: experimental design, hardware fabrication and characterisation, all 456 robot trials and data collection, quantitative data analysis, and the intellectual contributions described in \cref{sec:conclusion-summary}. The research questions, hypotheses, experimental results, failure mode classifications, and conclusions are the author's own.

This disclosure is made in accordance with the guidelines of the Karlsruhe Institute of Technology (KIT) on the use of AI tools in academic work. The author accepts full responsibility for the accuracy, integrity, and originality of all content in this thesis.
\newpage
...
% --- Abstract (English) ---
\begin{abstract}
With the increasing degree of automation in the manufacturing industry, robotic systems have become a key topic. A manipulator module which features a robot, a mobile workstation and the computing capability to finish a flexible pick-and-place assignment and deliver materials for the intralogistics systems is a key enabler for the future circular factory.

The work of this thesis is to implement an uncertainty-aware machine-learning-based vMF-Contact~\cite{shi2025} algorithm on the manipulator module. The robot handles an angle grinder in a computed tomography (CT) cell in the circular factory at Karlsruhe Institute of Technology (KIT). This thesis addresses two systematic limitations of the algorithm on a UR10e collaborative robot. Candidate generation lacks object-level physical priors, and deployment evaluation lacks a structured methodology.

Centroid-based physical constraints enter the vMF-Contact pipeline as a post-inference filter. An annular grip-zone bound, relative to the object's geometric centroid proxy, corrects grip force insufficiency. A gripper-axis alignment constraint eliminates placement orientation errors. A custom fingertip replaces the flat Robotiq 2F-140 tip. Its curved contact profile matches the angle grinder's cylindrical handle. A silicone friction layer addresses residual grip failures that persist after algorithmic correction. All adaptations are integrated into a reproducible ROS2 and Docker deployment pipeline. They are all presented as contributions of this thesis.

A factorial evaluation across 432 trials tested three conditions. This 3-stage experiment isolates the contribution of each adaptation. Unconstrained vMF-Contact achieves 13.9\,\% success. Centroid-based constraints raise this to 40.3\,\%. The custom fingertip raises it further to 77.1\,\%. The 77.1\,\% result exceeds the published vMF-Contact algorithm success rate of 72.7\,\%. This holds despite three compounding deployment disadvantages: lower grip force, twice the object weight, and a stricter pick-transport-place criterion. Failure mode analysis identifies constraint-induced kinematic infeasibility as the dominant residual failure mechanism. Another grasp reachability filter is implemented subsequently as a supplementary improvement.

\textbf{Keywords:} robotic grasping, industrial deployment, pick-and-place, collaborative robot, uncertainty-aware grasping
\end{abstract}
...
The mobile manipulator module, that is being commissioned and evaluated in this thesis, is part of the SFB-1574 Circular Factory at the Karlsruhe Institute of Technology (KIT). A circular factory is made up of three types of stations, i.e. production stations for remanufacturing products, inspection stations for checking products that have been used before and their condition, and an adaptive intralogistics system for material transport between the stations. The above-mentioned stations are supported by a fleet of mobile manipulators, equipped with various types of modules, such as buffers, for storing objects, transport modules for moving objects, and manipulator modules for processing the objects. An uncertainty-aware machine-learning-based algorithm vMF-Contact is implemented to the above-mentioned manipulator module. The algorithm will be explained in more detail in \cref{sec:uncertainty-aware-grasping}.

The evaluated robot station is a manipulator station picking, placing angle grinders delivered by an AGV to the placement zone of the CT-scanner for quality inspection. After quality inspection the AGV will be loaded with the angle grinder again and it will transfer the object to the next station. This task is one of the basic functions of the intralogistics in the Circular Factory. It is adaptive picking and manipulation of end-of-life products in production and in the quality inspection stations in presence of positional and orientational uncertainty.

The mobile manipulator module consists of 6 hardware components: the UR10e, the Orbbec Femto Mega RGBD camera, the Robotiq 2F-140 gripper, the mini-PC (suggested RAM > 16\,GB), the mobile workstation and a network switch. For the evaluation we use a Jetson Orin Nano Super Developer Kit 8\,GB and a personal laptop 8\,GB as supplementary hardware. An overview of the hardware components in a mobile manipulator module is shown in \cref{fig:hardware-overview}. Specifically, in this work the manipulator module is intended for implementation in a CT cell. To avoid positional and orientational deviation of the mobile manipulator module, a clamping station has also been designed.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{robot-setup-markup.png}
  \caption{Hardware overview. It shows the hardware components on the mobile manipulator module and the CT machine.}
  \label{fig:hardware-overview}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{connection-overview.png}
  \caption{Connection overview. It describes the different connection types between the mobile manipulator module and the CT cell environment, as well as the internal connection between the robot module and computing module.}
...
% A product image is shown in \cref{fig:ur10e}.
...
% A product image is shown in \cref{fig:orbbec-femto-mega}.
...
\textbf{Robotiq 2F-140 Gripper.} The Robotiq 2F-140 parallel-jaw gripper provides a maximum jaw opening of 140\,mm and a maximum grip force of 125\,N. The 2F-140 was selected over the Robotiq 2F-85 (85\,mm opening, 235\,N grip force) because the width of the grip handle of the angle grinder exceeds 85\,mm. This geometric requirement comes at the cost of lower maximum grip force, a trade-off that directly motivates the custom fingertip design described in \cref{sec:custom-fingertip-design}. The gripper communicates with the UR10e controller via Modbus TCP and is commanded from the ROS2 pipeline through a dedicated gripper driver node. 
% A product image is shown in \cref{fig:robotiq}
...
\textbf{Robot workstation.} To enable the collaborative robot to move around in the circular factory, a workstation is designed specifically for this work. The workstation is made of Bosch Rexroth 90\,mm $\times$ 90\,mm and 45\,mm $\times$ 45\,mm aluminium profiles. It has a size of 800\,mm $\times$ 440\,mm $\times$ 850\,mm. The internal storage space can hold the control box, the network switch and other necessary IO devices. 
% Its CAD model and actual photo is shown in \cref{fig:robot-workstation}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{clamping-real.jpg}
  \caption{Image of the clamping station.}
  \label{fig:clamping}
\end{figure}

\textbf{Clamping station.} To reduce the positional and orientational deviation of the mobile robot when it comes to the operating point, a clamping station which consists of a zero-point clamping module and two aluminium plates is designed. The model of the zero-point clamping module is LANG Technik Quick Point 96. It features four clamping screw holes. These holes are made to fasten the robot workstation, where 4 clamping screws are mounted on the frame of the workstation correspondingly. With this clamping station, it ensures the robot always works at the same place. A product image of LANG Technik Quick Point 96 and a CAD model of the clamping station is shown in \cref{fig:clamping}. 
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.3\textwidth]{ur-kinematcis.png}
...
\cref{tab:ur10e-dh-parameters} lists the manufacturer-published values of $a_i$, $d_i$, and $\alpha_i$ for the UR10e~\cite{universalrobots2024dh}. The joint angles $\theta_i$ are omitted from the table because they are the joint variables read from the arm's encoders at run time.
...
Substituting the \cref{tab:ur10e-dh-parameters} row's $a_i$, $d_i$, $\alpha_i$ together with the corresponding run-time $\theta_i$ into \cref{eq:dh-transform}, then chaining the six resulting matrices per \cref{eq:fk-chain}, gives the UR10e's forward kinematics in closed form.
...
Reachable workspace, joint limits, and singularities are the three constraints that determine whether an otherwise valid grasp candidate is executable. \cref{sec:software-fundamentals} describes how MoveIt~2 embeds this IK step inside its collision-aware trajectory planning. Failures of this planning step against these three constraints are the kinematic infeasibility failure mode reported in \cref{sec:failure-mode-definitions}.

\section{Software Fundamentals}
\label{sec:software-fundamentals}

\textbf{ROS2.} The Robot Operating System~2 (ROS2) is the middleware framework underlying the software stack. It organises the system as a graph of independent nodes exchanging messages over topics, services, and actions, built on the Data Distribution Service (DDS) standard for inter-process communication. This thesis uses the POS2 Humble Hawksbill long-term support release running on Ubuntu 22.04. Nodes for perception, inference, motion planning, and robot control communicate through this framework. The specific containerised arrangement of these nodes is described in \cref{sec:docker-configuration}.

\textbf{Orbbec~SDK.} The Orbbec~SDK is a cross-platform software development kit for Orbbec 3D depth cameras, providing a unified interface to access RGB, depth, IMU, and point cloud data. It enables developers to configure camera parameters, synchronize sensors, process depth images, and generate 3D point clouds. With its high-level APIs, sample applications, and support for robotics frameworks such as ROS2, the Orbbec~SDK simplifies the development of computer vision, robotics, and 3D perception applications.
...
\cite{shi2025} report 72.7\,\% success on a diverse-object experimental setup. They used a Robotiq 2F-85 gripper and objects up to $\sim$1\,kg. In that paper a process is counted as success, when the gripper could grasp the object up and hand it to the operator shortly after. It only checks whether the gripper closes stably. The result excludes downstream task success such as transport and placement.

\section{The Industrial Deployment Gap}
\label{sec:industrial-deployment-gap}

Strong results from the experiment do not guarantee reliable industrial deployment. The Amazon Picking Challenge~\cite{correll2016analysis} was an early structured real-world test. It involved picking from cluttered, variably lit shelves. Experimental success did not always mean real-world reliability. This made deployment evaluation its own research concern.
...
Deployment success is more than just sensor noise. vMF-Contact's~\cite{shi2025} 72.7\,\% result differs from this deployment in three ways. First, the gripper that was used in this paper was Robotiq 2F-85 gripper, which delivers 235\,N. This thesis's Robotiq 2F-140 delivers only 125\,N, a 47\,\% drop. It was chosen for its wider jaw opening. Second, the objects that were lifted in this paper weigh up to $\sim$1\,kg. The angle grinder here weighs about $\sim$2\,kg instead. Third, the paper measures only isolated grasp success. This thesis measures full pick-transport-place success instead. That metric adds trajectory execution and placement accuracy. Each difference independently lowers the expected success rate.
...
\textbf{Gap A --- Missing physical priors in vMF-Contact.} Learning-based methods encode local contact geometry only. They ignore object-level physical priors entirely. vMF-Contact does not constrain grasp-point-to-centroid distance. It also does not constrain gripper-axis alignment. Asymmetric objects then cause systematic failures. Extremity grasps far from centroid cause grip force insufficiency. Misaligned axis grasps cause orientation inconsistency.

% \textbf{Gap B --- Absence of systematic deployment evaluation.} The literature offers isolated grasp results only.  Downstream task success is rarely the primary criterion.

\textbf{Gap B --- Absence of downstream tasks success evaluation.} Previous literature on grasping primarily reports isolated grasp results only, but it rarely evaluates whether the object is then successfully transported, placed, or otherwise completed in the downstream task. In real industrial applications, however, grasping is typically part of a larger pick-and-place workflow. It also lacks structured deployment studies with failure mode analysis and real hardware-object constraints as criteria.
% This thesis addresses this gap by evaluating post-grasping success as part of the full deployment process rather than relying on isolated grasp success alone.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{angle-grinder-spherical-grip-zone-cog.png}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{angle-grinder-spherical-grip-zone-axis-caption.png}
    ...
    \end{subfigure}
    \caption{Figures of centroid constraints applied in this work.}
    \label{fig:main}
\end{figure}
...
where $\hat{c}$ is the geometric centroid proxy, $c_{\mathrm{obb}}$ is the centre of the object's oriented bounding box, and $c_{\mathrm{pcd}}$ is the centroid of the filtered point cloud. The 0.8 weighting towards the point cloud centroid reflects that the measured surface distribution provides a closer approximation to the object's mass distribution than the bounding box centre for an asymmetric object Tthe 0.2 bounding box contribution acts as a regulariser, stabilising the estimate against noisy outliers. This weighting was validated against the angle grinder's physical geometry by confirming that the resulting centroid proxy fell consistently within the mechanically stable grip zone across all six object positions. This approximation is a geometric proxy for the centre of gravity, not a physical measurement of it. This geometric proxy is referred to throughout this thesis as the \textit{geometric centroid proxy}.

The grip zone boundary values applied in this work are:
\begin{itemize}
...
The filter that is introduced in this section, which includes centroid distance inner and outer bound and axis projection threshold, constrains candidates to positions that are mechanically sufficient and orientationally consistent, addressing grip force insufficiency and orientation errors. It does not evaluate whether the selected candidate is kinematically reachable by the UR10e before committing to execution. A candidate that satisfies this filter may still fail if the corresponding robot configuration falls outside the reachable workspace or violates joint limits. A second filter that evaluates kinematic reachability prior to final candidate selection --- pre-emptively discarding unreachable poses and selecting the next viable candidate --- is identified as a direction for future work (\cref{sec:future-work}) and implemented in Appendix \cref{sec:appendix-filter-design}.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\linewidth]{docker-structure-full.jpg}
  \caption{Multi-layer docker container structure }
  \label{fig:docker-container-structure}
\end{figure}

The software stack runs on ROS2 Humble (Ubuntu 22.04) and is structured as a Docker container managed by Docker Compose. It consists of 4 image layers: Source image, ROS2 base image, 1.overlay image and 2.overlay image as shown in \cref{fig:docker-container-structure}. Containerisation includes all dependencies and environment in a single final docker image, enabling fast deployment across development workstations with a simple "docker compose up" command.

As we are using a Jetson Orin Nano together with a laptop in this work, we had the container on both of the devices. Containers communicate over ROS2's DDS middleware (FastDDS) on a shared host network namespace, maintaining ROS2 node discovery without inter-container bridging. It serves as the software fundamentals for the perception-inference-execution pipeline shown in \cref{fig:software-overview} and \cref{fig:vmf-pipeline}. 

As shown in \cref{fig:software-overview}, the vMF-pipeline includes 5-stage nodes. The base node runs ROS2 as fundamental. The perception node processes and passes the original point cloud to the subsequent preprocessing node. The preprocessing node converts the point cloud in to the robot base frame via TF-transformation and passes it down. The inference node subscribes, runs inference and constraint filtering, and publishes the selected grasp pose in the robot base frame. The motion planning node computes a collision-aware trajectory using the UR10e URDF. The robot control node executes the trajectory and reports execution status. A logging node, co-located in the inference node, records per-trial metadata for each attempt: inference time, geometric centroid proxy, grasp pose, candidate count before and after filtering and point cloud logging.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{robot-setup-and-experiment-protocol.png}
  \caption{This figure shows the experimental setup and protocol of this work. The angle grinder is placed at one of six positions and one of eight orientations for each trial.}
...
%   \caption{Inference result of 6 object positions P1--P6 used in the evaluation.}
...
%   \caption{Inference result of 8 object orientations O1--O8 used in the evaluation.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\linewidth]{angle-grinder-holder-tolerance.png}
...
The transition from Baseline~1 to Baseline~2, which shows an improvement of 26.4\,\%, represents the software contribution. Centroid-based constraints filter out candidates at object extremities and enforce axis alignment, directly addressing both dominant Baseline~1 failure modes. This improvement is attributable entirely to the algorithmic modification; hardware is held constant.

The transition from Baseline~2 to Final, which shows an improvement of 36.8\,\%, represents the hardware contribution. The custom fingertip increases effective contact area and friction on the angle grinder handle, recovering grip failures on candidates that are geometrically valid but mechanically insufficient with the flat fingertip. This improvement is attributable entirely to the hardware modification; the algorithm is held constant.
...
\subsection{Comparison to vMF-Contact Algorithm Baseline}
\label{sec:benchmark-comparison}

The Final condition result of 77.1\,\% demonstrates viability under conditions more demanding than those of the vMF-Contact baseline results~\cite{shi2025} along three independently measurable axes:

(1)~the Robotiq 2F-140 grip force (125\,N) is 47\,\% lower than the 2F-85 (235\,N) used in the vMF-Contact baseline;

(2)~the angle grinder ($\sim$2\,kg) is approximately twice the weight of vMF-contact baseline objects;

(3)~the success criterion encompasses the full pick-transport-place sequence, whereas the vMF-Contact baseline measures isolated grasp success.

Each factor independently would depress the achievable success rate relative to the vMF-Contact baseline setting. That the Final condition achieves 77.1\,\% despite these three compounding disadvantages indicates that the centroid-based constraints and custom fingertip are sufficient to maintain above-baseline performance even under harder deployment conditions.
...
A valid candidate is selected, but the MoveIt~2 planner fails to find a collision-free trajectory or certain joints exceed their limits (\cref{fig:failure-mode-F3-no-trajectory} and \cref{fig:failure-mode-F3-joint-limits}). Reported by the planning node or robot controller.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\linewidth]{F3-collision-trajectory.png}
  \caption{Failure mode F3 in MoveIt~2: Kinematic infeasibility. Target pose is
    received, but the motion planner fails to find a collision-free trajectory or certain joints exceed their limits.}
  \label{fig:failure-mode-F3-no-trajectory}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\linewidth]{F3-alart-joint-limit.jpg}
  \caption{Failure mode F3 on UR10e: Kinematic infeasibility. Target pose is
    received, but certain joints exceed their limits.}
  \label{fig:failure-mode-F3-joint-limits}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Failure mode F4 in visualization. Object grasped at inverted orientation.}
...
\begin{figure}[htbp]
  ...
  \caption{Failure mode F4 in real scene. Object placed at inverted orientation.}
...
\textbf{Epistemic uncertainty and low-confidence orientations lead to F1 before adapting the fingertip.} In Baseline~2 (\cref{tab:baseline2-orientation-clusters}), orientations O1, O2, O5, O6, and O7 produce elevated F1 rates (9--16 failures per orientation out of 18 trials). These orientations yield few surviving candidates after centroid filtering(typically one to two candidates) compared to five to ten at well-represented orientations. The result shows the impact of epistemic uncertainty on the inference, as the point clouds at these orientations are usually incomplete from a single viewpoint. The vMF-Contact model has limited training coverage of these viewpoints, producing a low-density distribution that generates few high-confidence candidates within the centroid zone. The system selects from a small, low-confidence pool, increasing the probability of selecting a candidate at a marginal contact position where the flat fingertip provides insufficient grip.
...
Baseline~1 achieves 13.9\,\% success. Baseline~2 raises this to 40.3\,\%. Final condition raises it further to 77.1\,\%. The 77.1\,\% result exceeds the published success rate of vMF-Contact algorithm of 72.7\,\% despite three compounding deployment disadvantages: 47\,\% lower grip force, twice the object weight, and a stricter full pick-transport-place success criterion. Failure mode analysis identifies constraint-induced kinematic infeasibility as the dominant residual failure mechanism.
...
\textbf{Gap B --- Absence of downstream tasks success evaluation.} Prior work provides grasping success rate comparisons but not structured deployment studies with failure mode analysis, real hardware-object constraints, and a full pick-transport-place success criterion. The 432-trial evaluation across three conditions, eight orientations, and six positions fills this gap for the specific deployment studied here. The failure mode analysis demonstrates under what condition a validated method succeeds and fails in a real industrial cell.
...
In the Final condition, kinematic infeasibility (F3) is the dominant residual failure mode: 25 of 144 trials (17.4\,\%) end in motion-planner failure. Two orientations account for 96\,\% of these failures: O3 (15 of 18 trials) and O7 (9 of 18 trials). O3 achieves zero successes across all three experimental conditions.
...
\begin{figure}
  \centering
  \includegraphics[width=0.8\linewidth]{filter-cascade-diagram.png}
  \caption{This figure shows the filter cascade architecture.}
  \label{fig:filter-cascade}
\end{figure}

The reachability filter extends the existing constraints set of \cref{sec:vmf-contact-inference} with two additional directional constraints on the grasp approach geometry. A detailed filter cascade diagram is shown in \cref{fig:filter-cascade}. 

The newly added constraints are geometric heuristics. They use simple vector operations computed from the candidate pose and robot frontal axis, without invoking the IK solver or motion planner. 

\paragraph{Filter B1 — Frontal projection constraint.}

\begin{figure}
  \centering
  \includegraphics[width=0.6\linewidth]{frontal-overview-1.png}
...
\begin{figure}
  \centering
  \includegraphics[width=0.8\linewidth]{vertical-grasp.png}
...
There are three level filter switches in the final filter cascade. The \param{grasp\_cog\_max\_dist\_th} and \param{grasp\_cog\_max\_dist\_th} are the first level switch. By setting value to them, grasp zone will be limited to a certain area around the geometric centroid proxy, as described in \cref{sec:centroid-distance-constraints}. Otherwise, the node will work as the base vMF-Contact algorithm and no grasp candidates will be filtered. \param{use\_reachable\_grasp\_filter} is the second level switch to this cascade, which determines whether to use the filter B1 and B2 introduced in this chapter. It is parallel to Filter A, which is introduced in \cref{sec:axis-alignment-constraint}. But only one group of filter can take in effect. This is the function of the third level filter \param{use\_cog\_filter}. They cannot exist at the same time as the axis alignment constaints is not compatible with the front direction constraints. With Filter A, grasp poses whose gripper y-axis is aligned with object principle axis are passed down. With Filter B1, grasp poses whose gripper y-axis is aligned with robot frontal axis are passed down. And candidate set will be further filtered based on B2. Those with gripper z-axis orientation out of the threshold will be filtered.

\section{Experimental Results}
\label{sec:appendix-results}

The evaluation uses a reduced factorial matrix: 8 object orientations (O1--O8) $\times$ 1 delivery position $\times$ 3 repetitions per cell, yielding 24 trials in total. The hardware and software configuration matches the Final condition: centroid-based constraints active ($d_{\max}$\,=\,0.025\,m, $d_{\min}$\,=\,0.01\,m, $th_{axis\_proj}$\,=\,0.5), custom curved silicone fingertips, UR10e + Orbbec Femto Mega + Robotiq 2F-140. The success criterion follows \cref{sec:success-criterion} (full pick-transport-place completion without object drop). The previously defined failure mode \textbf{F4} is now counted as a partial success (\textbf{PS}). It describes trials where the object is placed inverted relative to the intended target pose. It is a tradeoff of newly introduced filters. \cref{sec:appendix-limitations} discusses what this category reflects.
...
The fontal projection constraint (Filter B1) filters candidates whose gripper y-axis poses to the frontal direction of the robot station, which is the direct mechanism for the F3 elimination at these two orientations. Unlike the anticipated risk that a tighter candidate set at O3 and O7 would convert F3 trials into F2 trials, no F2 failures occur in the reduced matrix. The surviving candidate sets at every orientation retained at least one candidate that satisfied both new filters.  The vertical approach constraint (Filter B2, $\theta_{\max} = 10^{\circ}$) removes tilted gripper approach direction candidates before the motion planner is invoked.
...
% The two new filters are geometric heuristics, not exact kinematic feasibility checks. On the one side, a candidate that passes both filters is not guaranteed to be reachable. On the other side, a candidate that fails one filter may in some cases be reachable via an alternative IK solution. 

\textbf{$\theta_{\max}$ threshold is an empirical value.}. The $10^{\circ}$ threshold for the vertical approach constraint was set empirically based on observation of F3 failure poses and has not been optimised over a parameter grid. A too tight threshold would increase F2. One that is too loose provides insufficient F3 reduction. The precise operating range of the threshold is an open question for future work.

\textbf{Position sensitivity is not investigated.} The reachability filter was evaluated at a single position. Although it has been discussed in \cref{sec:failure-distribution-by-position} that in previous experiments the failures show no position-specific clustering, it is based on the filter A. Object Position may affect the completeness of the object in point cloud as well, if the camera always stays at the same viewpoint. Position-dependent effects are not evaluated in this set of expreiment and remain a direction for further investigation.

\textbf{Inverted placement comes as a tradeoff of the reachability filter} Eliminating F1--F3 does not come without cost. Of the 24 trials, 13 are placed inverted relative to the intended target pose (\textbf{PS}); only O4, O6 and O8 are entirely free of a placement defect. O3, O5, and O7 are inverted in all three trials.

This is a direct tradeoff of the frontal projection constraint (Filter B1, Eq.~\eqref{eq:frontal-projection}). Filter B1 selects candidates whose gripper y-axis projects positively onto the robot's frontal direction $\mathbf{f}$, without reference to which end of the object's principal axis the candidate sits on. Filter B1 can select the grasp pose that satisfies the reachability filter rather than the one consistent with the fixed object principal axis. Because the placement stage executes a single hardcoded target pose regardless of which end of the object was grasped, a candidate grasped at an inverted orientation will also arrives inverted at the delivery position. Since the inversed placement does not have a corresponding placement target pose, in some cases the object must be manually reoriented after placement to be usable, depending on the use case.

This tradeoff is acceptable for the current implementation. No trial drops the object ($\text{F1}$), fails to reach a valid pose ($\text{F2}$), or exceeds the robot's kinematic limits ($\text{F3}$). The inverted orientation is acceptable for the subsequent CT inspections in our case, as long as it lands on the holder. Exchanging a hard failure (F3, requiring a full retry cycle) for an immediately observable placement defect is a net improvement in a supervised deployment setting, even though it introduces a correction burden that Filters B1 and B2 alone do not remove. A flexible placement stage that recognizes the orientation and grasp point of the object being grasped and gives out a corresponding placement pose, rather than relying on a single hardcoded target pose, is left for future work.
