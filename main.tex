...
\textbf{TODO} n this section the circular factory should be introduced, including the AGV delivery system and the UR10e deployment cell. The angle grinder should be described as a representative object with asymmetric geometry and a handle that requires a parallel-jaw gripper with a wide jaw opening. The limitations of the Robotiq 2F-140 gripper --- lower grip force than the 2F-85, but wider opening --- should be highlighted. The vMF-Contact algorithm should be introduced as a state-of-the-art learning-based grasp generation method that produces a ranked candidate set of 6-DoF grasp poses from a single point cloud. Its probabilistic formulation over contact normals should be described, along with the distinction between aleatoric and epistemic uncertainty. The benchmark success rate of 72.7\,\% on diverse objects should be cited, along with the fact that this benchmark does not include object transport or placement accuracy. The gap between benchmark performance and real deployment success should be highlighted, motivating the research questions and contributions of this thesis.
...
\begin{figure}
  \centering
  \includegraphics[width=0.95\linewidth]{partially_success_grasp.png}
...
F3 increases from 14 to 25 (17.4\,\%): driven by O3 (12$\rightarrow$15) and O7 (1$\rightarrow$9). The orientation-specific F3 pattern is examined in Section~4.4.
...
\begin{table}[htbp]
  ...
  \end{tabular}
\end{table}
\textbf{TODO: add clusters for position-level failure distribution}
