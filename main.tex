...
The reachability filter extends the three-filter cascade of Section~3.4 with two additional directional constraints on the grasp approach geometry. Both checks are geometric heuristics — lightweight vector operations computed from the candidate pose — that pre-discard approach directions associated with workspace-boundary configurations without invoking the IK solver or motion planner.

\paragraph{Filter 4 — Frontal projection constraint.}
The UR10e's nominal frontal direction is defined as $\mathbf{f} = [-1,\,0,\,0]^{\!\top}$ in the world frame. For each surviving candidate, the projection of the gripper closing axis $\hat{a}$ onto $\mathbf{f}$ is computed:
\begin{equation}
  p = \hat{a} \cdot \mathbf{f}.
\label{eq:frontal-projection}
\end{equation}
Candidates with $p \leq \texttt{grasp\_axis\_projection\_th} = 0$ are discarded. The threshold of zero requires a strictly positive projection, ensuring the gripper faces into the robot's primary workspace. Grasps directed away from the robot require shoulder and elbow joints to approach their limits in order to reach the target pose.

\paragraph{Filter 5 — Vertical approach constraint.}
The gripper's approach axis $\hat{g}_{z}$ (the z-axis of the gripper frame) is constrained to remain within $\theta_{\max}$ of the downward vertical $-\hat{z}_{w}$:
\begin{equation}
  \angle\!\left(\hat{g}_{z},\,-\hat{z}_{w}\right) \;\leq\; \theta_{\max}.
\label{eq:vertical-approach}
\end{equation}
The threshold $\texttt{grasp\_z\_negative\_world\_z\_angle\_th} = 10^{\circ}$ is applied. Candidates exceeding this angle are discarded.
...
Filters~4 and~5 are inserted after Filter~3 (axis alignment, Section~3.4.3) and before motion planning, forming a five-stage cascade:

\begin{enumerate}
  \item Centroid distance outer bound (\textit{prevents grip-force insufficiency, F1})
  \item Centroid distance inner bound (\textit{prevents grip-force insufficiency, F1})
  \item Axis alignment with principal axis (\textit{prevents placement orientation error, F4})
  \item \textbf{Frontal projection} — Eq.~\eqref{eq:frontal-projection} \quad\textit{(reachability heuristic, F3)}
  \item \textbf{Vertical approach angle} — Eq.~\eqref{eq:vertical-approach} \quad\textit{(reachability heuristic, F3)}
\end{enumerate}

Both new filters are vector arithmetic with negligible runtime cost. The pipeline latency increase relative to the Final condition is below 1\,ms per inference call. The implementation is in branch \texttt{dev\_3} of the vMF-Contact fork:
\begin{center}
...
The evaluation uses a reduced factorial matrix: 8 object orientations (O1--O8) $\times$ 1 delivery position $\times$ 3 repetitions per cell, yielding 24 trials in total. The hardware and software configuration matches the Final condition: centroid-based constraints active (\param{grasp\_cog\_max\_dist\_th}\,=\,0.025\,m, \param{grasp\_cog\_min\_dist\_th}\,=\,0.01\,m, \param{cog\_axis\_projection\_th}\,=\,0.5), custom curved silicone fingertips, UR10e + Orbbec Femto Mega + Robotiq 2F-140. The success criterion is identical to Section~4.1.3: full pick-transport-place completion without object drop or inverted placement.

The reduced matrix isolates orientation effects at a single position. Per-orientation results are directly comparable to the Final condition per-orientation counts in Table~4.9 at the evaluated position, isolating the effect of the reachability filter from position-level variation.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrrr}
    \toprule
    Orientation & F1 (grip) & F2 (no pose) & F3 (kinematic) & F4 (orient.) & Success \\
    \midrule
    O1 & & & & & \\
    O2 & & & & & \\
    O3 & & & & & \\
...
% TODO: fill Table A.1 from experimental logs once evaluation is complete.

\section{Analysis}
\label{sec:appendix-analysis}

% TODO: replace the expected-effects paragraph below with actual analysis once
%       Table A.1 is complete. Keep the mechanistic explanation in A.6.1.

\subsection{Expected Effects of the Reachability Filter}
\label{sec:appendix-expected}

The two new filters act on different failure mechanisms with predictable directional effects on the failure distribution.

\textbf{F3 at O3 and O7.}
The vertical approach constraint (Filter~5, $\theta_{\max} = 10^{\circ}$) directly targets the mechanism that produces F3 at O3 and O7: handle-region candidates at those orientations tend to carry tilted approach vectors. Pre-filtering tilted candidates reduces the probability that the top-ranked surviving candidate requires a workspace-boundary joint configuration. F3 at O3 and O7 is expected to decrease; whether it reaches zero depends on whether any handle-region candidate at those orientations satisfies the $10^{\circ}$ constraint.

\textbf{F2 at O3 and O7.}
Each additional filter reduces the surviving candidate set. At orientations where the centroid filter already leaves few candidates (Section~4.4), Filters~4 and~5 may eliminate all remaining candidates, converting F3 trials into F2 trials. F2 is detectable before motion planning and can be operationally mitigated (retry, reposition); F3 requires a complete planning cycle to detect. Even if total failures do not decrease, a shift from F3 to F2 improves operational observability.

\textbf{F1 and F4.}
Filter~4 (frontal projection) is orientation-symmetric with respect to the object's principal axis; it does not introduce a preferred placement direction. Filter~5 (vertical approach) constrains the approach angle but not the closing-axis direction. Neither filter is expected to affect F1 (grip force) or F4 (orientation error), which are governed by centroid zone membership and axis alignment respectively.

\textbf{Productive orientations (O1, O2, O4, O5, O6, O8).}
The Final condition already achieves 94.4\,\% success over these six orientations (102/108 trials over the full matrix). Handle-region candidates at productive orientations already tend toward near-vertical approach vectors, so Filter~5 is expected to discard few candidates. The net effect at productive orientations is expected to be small.
...
The reachability filter was evaluated at a single position. Position-dependent effects on the filter's performance — arising from the object's absolute location relative to the robot base frame — are not characterised in this evaluation and remain a direction for further investigation.
