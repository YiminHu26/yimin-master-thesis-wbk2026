...
\begin{figure}[htbp]
  \centering
  \fbox{\parbox{0.8\textwidth}{\centering\vspace{3cm}
    \textit{[Figure 3.2: Annular grip zone on the angle grinder point cloud. Inner radius $d_{\min}$ = 0.01\,m, outer radius $d_{\max}$ = 0.025\,m, centred on geometric centroid proxy $\hat{c}$.]}
    \vspace{3cm}}}
...
\begin{table}[htbp]
  \centering
  \caption{Overall success rates per condition (144 trials each). Enter exact success counts from experimental logs.}
...
The transition from Baseline~1 to Baseline~2 --- an improvement of approximately 45 percentage points --- represents the software contribution. 
...
The transition from Baseline~2 to Final --- an improvement of approximately 30 percentage points --- represents the hardware contribution. 
...
The Baseline~2 result (13.9\,\%) is insufficient for reliable industrial operation; 
...
\textbf{Baseline~1.} 
F1 dominates: candidates placed at object extremities generate insufficient grip force, resulting in release during transport. F4 accounts for a secondary fraction. 
F2 does not occur (no filter is active). 
F3 occurs at a rate determined by workspace geometry and is consistent across all three conditions.

\textbf{Baseline~2.} 
F1 is substantially reduced. 
Centroid constraints concentrate candidate selection in the mechanically stable handle region, eliminating the majority of Baseline~1 grip failures. 
Residual F1 occurs where the flat fingertip fails to maintain grip on a valid candidate. 
F2 appears at a small number of cells. 
F4 is reduced by the axis alignment filter. 
F3 is unchanged.

\textbf{Final.} 
F1 is further reduced by the custom fingertip. 
Residual F1 is concentrated at orientations with low candidate counts. 
F4 frequency is similar to Baseline~2 --- the fingertip modification does not affect orientation compliance. 
F2 and F3 rates have slightly increased because of randomness of the object position and environment.
...
\textbf{F3 cluster.} 
A subset of orientations produces consistently elevated F3 rates across all conditions and positions (see Table~\ref{tab:baseline1_orientation_clusters}, \ref{tab:baseline2_orientation_clusters}, \ref{tab:final_orientation_clusters}). 
These orientations require joint configurations at or near the UR10e's joint limits, or Cartesian approach directions that intersect the workspace boundary. 
F3 failures at these orientations are not attributable to the algorithm or the fingertip --- they reflect the robot's kinematic envelope, and are unchanged whether Baseline~1, Baseline~2, or the Final condition is active. 
They are included in the full-matrix success rate denominator, as they represent real failure conditions in the deployment. 
For reference, the success rate over kinematically-feasible orientations only --- excluding cells dominated by F3 --- is \textbf{94.4\,\%} in the Final condition, compared to 77.1\,\% over the full matrix. 
The pre-emptive reachability filter described in Section~3.4.5 is the appropriate solution to this cluster and is identified as future work.
...
A second cluster of orientations produces elevated F1 rates in both Baseline~2 and the Final condition. 
These orientations yield few surviving candidates after CoG filtering --- typically one to two candidates, compared to five to ten at well-represented orientations. 
The sparse candidate set is consistent with high epistemic uncertainty: 
the vMF-Contact model has limited training coverage of these viewpoints, producing a broad, low-density distribution that generates few high-confidence candidates within the centroid zone. 
The system selects the best available candidate from a small, low-confidence pool; this candidate is more likely to fall on a marginal contact position where even the custom silicone fingertip does not provide reliable grip. 
The custom fingertip reduces the F1 rate at these orientations relative to Baseline~2, but does not eliminate it. 
Orientation sensitivity therefore persists into the Final condition as a residual effect of model coverage, rather than of hardware or constraint design.
...
Position primarily influences F2 incidence (Section~4.3.3); it has no measurable effect on F3 or on the F1 cluster pattern.
...
Integrating and validating this filter on the current trial matrix is the most direct path to reducing F3 failures and improving the full-matrix success rate beyond 80\,\%.
...
The gap between 72.7\,\% (benchmark) and 11.9\,\% (this deployment without adaptation) is not attributable to any single factor --- it is the compound product of missing geometric priors, hardware-object force mismatch, and a stricter success criterion operating together.
